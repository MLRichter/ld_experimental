...
def train(gpu_id, world_size, n_nodes):
    ...
    if is_main_node: # <--- DDP
        ...
        wandb.init(project=wandv_project, name=wandb_run_name, entity=wandv_entity, id=run_id, resume="allow", config=wandb_config)
    ...
    if is_main_node: # <--- DDP
        if checkpoint is not None:
            ...
            wandb.alert(title=f"Training {run_id} resumed", text=f"Training {run_id} resumed from step {checkpoint['scheduler_last_step']}")
        else:
    ...
    for it in pbar:
        ...
        with torch.no_grad():
            ...
            if rand_idx.any():
                clip_image_embeddings[rand_idx] = clip_image_model(clip_preprocess(images[rand_idx])).pooler_output.unsqueeze(1)
            
            t = (1-torch.rand(images.size(0), device=device)).add(0.001).clamp(0.001, 1.0)
            latents = vqmodel.encode(images)[0]
        ...
        if is_main_node and (scheduler.last_epoch == 1 or scheduler.last_epoch % print_every == 0 or it == max_iters): # <--- DDP
            if np.isnan(loss.mean().item()):
                ...
                wandb.alert(title=f"Skipping sampling & checkpoint for training run {run_id}", text=f"Skipping sampling & checkpoint at {scheduler.last_epoch} for training run {run_id} iters because loss is NaN")
            else:
                ...
                with torch.no_grad():
                    ...
                    noised_images = vqmodel.decode(noised_latents).clamp(0, 1)
                    pred_images = vqmodel.decode(pred).clamp(0, 1)
                    sampled_images = vqmodel.decode(sampled).clamp(0, 1)
                generator.train()
                ...
                log_data = [[captions[i]] + [wandb.Image(sampled_images[i])] + [wandb.Image(images[i])] for i in range(len(images))]
